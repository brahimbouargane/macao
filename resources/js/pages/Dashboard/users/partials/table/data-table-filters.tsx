...
export default function DataTableFilters<TData>({
  ...
  // get allowed filters
  const usersTableFilterColumns = table.getAllColumns().filter((column) => {
  ...
  // append existing query string filters to appear in the dropdown menu ui
  useEffect(() => {
    builder.tap((state) => {
      markFilterAsActive({
        filters: ['id', 'name', 'email', , 'role', 'created_at', 'updated_at', 'created_by', 'last_updated_by'],
        state,
        toggleFilter
      });
    });
    ...
    <>
      <Menu isOpen={open} onOpenChange={setOpen}>
        ...
        <Menu.Content placement="bottom right" selectionMode="multiple" className="dark:bg-accent">
          ...
          {usersTableFilterColumns.map((column) => {
            return (
              <Menu.Item
                key={column.id}
                className={
