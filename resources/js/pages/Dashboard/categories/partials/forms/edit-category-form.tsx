...
export default function EditCategoryForm({ setIsModalOpen, category }: EditCategoryFormProps) {
  const { categories } = useQueryBuilderCategoriesContext();
  const translations = usePage<PagePropsData>().props.translations;

  const selectedParentCategoriesIds = useListData<CategoryData>({
    initialItems: category.parentCategories
  });

  const { errors, data, setData, processing, clearErrors, post, reset, setError } = useForm({
    name: category.name,
    description: category.description ?? null,
    image: category.optimizedImage ? [category.optimizedImage] : [],
    selected_ParentCategoriesIds: category.parentCategories ? category.parentCategories?.map((s) => s.id) : [],
    _method: 'patch'
  });
            ...
            >
              {droppedImage ? (
                ...
                <AvatarUploadPreview
                  ...
                  shape="square"
                  src={data.image ? data.image : category.optimizedImage}
                />
              )}
            ...
            <div>
              ...
              {errors.selected_ParentCategoriesIds && errors.selected_ParentCategoriesIds.length > 0 && (
                <div className="text-sm text-danger forced-colors:text-[Mark]">
                  {errors.selected_ParentCategoriesIds}
                </div>
              )}
            </div>
          </div>
