...
const Sidebar = ({
    ...
    return (
      <Sheet isOpen={openMobile} onOpenChange={setOpenMobile} {...props}>
        <Sheet.Content
          ...
          side={side}
        >
          <Sheet.Body className="p-0 sm:p-0 !gap-0">{children}</Sheet.Body>
        </Sheet.Content>
      </Sheet>
    );
  }
  return (
    ...
    >
      ...
      <div
        className={cn(
          'duration-200 fixed inset-y-0 z-20  h-svh w-[--sidebar-width] transition-[left,right,width] ease-linear flex',
          side === 'left'
      ...
      >
        <div
          data-slot="sidebar"
          className={cn(
            'flex h-full w-full flex-col bg-bg group-data-[intent=inset]:bg-transparent group-data-[intent=floating]:rounded-lg group-data-[intent=floating]:border group-data-[intent=floating]:border-border group-data-[intent=floating]:bg-secondary/50',
...
const itemStyles = tv({
  base: [
    'group/sidebar-item grid cursor-pointer [&>[data-slot=icon]]:size-4 col-span-full [&>[data-slot=icon]]:shrink-0 items-center [&>[data-slot=icon]]:text-muted-fg relative rounded-lg lg:text-base leading-6',
  ...
  variants: {
    collapsed: {
      true: [
        ...
        'md:place-content-center md:grid-cols-[auto] md:[&>[data-slot=icon]]:mr-0 md:px-0 md:py-0 size-9'
      ],
    ...
    isFocusVisible: {
      true: 'bg-muted [&:focus-visible_[slot=description]]:text-primary/70 text-secondary-fg'
    },
    isHovered: {
      true: [
        '[&_[data-slot=icon]]:text-primary [&_.text-muted-fg]:text-primary/80 bg-accent-subtle text-primary'
      ]
    },
    isCurrent: {
      true: [
        '[&_[data-slot=icon]]:text-primary [&_.text-muted-fg]:text-primary/80 bg-accent-subtle text-primary',
        '[&_.bdx]:bg-primary/20 [&_.bdx]:ring-primary/30'
      ]
    },
    isDisabled: {
  ...
  return state === 'collapsed' && !isMobile ? (
    <Tooltip closeDelay={0} delay={0}>
      <Link
        {...props}
        className={cn(
          'grid rounded-lg focus:outline-none col-span-full   size-9 place-content-center hover:bg-accent-subtle hover:text-primary',
          isCurrent && 'text-primary bg-accent-subtle'
        )}
      >
